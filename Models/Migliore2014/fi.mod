...
INITIAL {
    ...
    printf("tau1 %g, tau2 %g, init factor: %g, tau1/tau2: %g \n",tau1,tau2,factor,tau1/tau2)
}
...
DERIVATIVE state {
    ...
    printf("t %g, i %g, g %g, B %g, A %g, v %g, e %g, gmax %g\n", t,i,g,B,A,v,e,gmax)
}
...
NET_RECEIVE(weight, s, w, tlast (ms)) {
    ...
    printf("NET REC XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n")
    INITIAL {
		:s = 0
        printf("nr initial\n")
		w = weight*plast(s)
    ...
    printf("s %g,tlast %g, B %g, B+r %g, ",s,tlast,B,B + w*factor)
	A = A + w*factor
	B = B + w*factor
