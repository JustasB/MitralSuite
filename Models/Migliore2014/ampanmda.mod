...
INITIAL {
    ...
    printf("Cdur %g, Alpha %g, Beta %g, E %g, mg %g, gmax %g, gampafactor %g, nmdafactor %g, ltdinvl %g, ltpinvl %g, sighalf %g, sigslope %g, ampatau %g",Cdur, Alpha, Beta, E, mg, gmax, gampafactor, nmdafactor, ltdinvl, ltpinvl, sighalf, sigslope, ampatau)
}
...
NET_RECEIVE(weight, s, w, tlast (ms), r0, t0 (ms)) {
    ...
    if (flag == 0) { : a spike, so turn on if not already in a Cdur pulse
        ...
        Ron = Ron - r0
        Roff = Roff + r0
        printf("t %g afterspike XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n",t)
	}
}
